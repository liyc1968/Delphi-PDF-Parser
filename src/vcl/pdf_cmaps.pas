...
function
pdf_get_wmode(cmap:ppdf_cmap_s):integer;
begin
	result:= cmap^.wmode;
end;
...
begin
	...
	if (cmap^.tlen + len + 1 >= USHRT_MAX) then
		dddd  //fz_warn("cannot map one to many; table is full");
	else
	begin
		offset := cmap^.tlen;
		add_table(cmap, len);
...
begin
  ...
  inc(b);
	...
	while integer(b) < integer(@(pdf_range_s_items(cmap^.ranges)[cmap^.rlen])) do
	begin
	...
	cmap^.rlen := (integer(a) - integer(cmap^.ranges)) div sizeof(pdf_range_s)+ 1;
	...
	begin
		...
		begin
			...
			begin
				...
				begin
					cpt^ := c;
					result:= pointer(integer(buf) + n + 1);
				end;
			end;
		end;
	end;
